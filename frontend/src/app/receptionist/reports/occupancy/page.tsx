...
import { CalendarDays, Hotel, TrendingUp, Save, Crown } from 'lucide-react';
...
export default function OccupancyReportPage() {
  ...
  const fetchOccupancyReport = async (saveReport: boolean = false) => {
    ...
    try {
      ...
      if (saveReport) {
        setSaveSuccess(t('occupancyReportSaved') || 'Report saved successfully!');
        setNote('');
      }
    } catch (err: any) {
      setError(err.response?.data?.message || t('failedFetchOccupancy') || 'Failed to load report');
    } finally {
      setLoading(false);
    }
  };

  // ROYAL LOADING SCREEN — FIXED & BEAUTIFUL
    ...
    return (
      <div className="fixed inset-0 bg-gradient-to-br from-amber-950 via-emerald-950 to-black flex items-center justify-center overflow-hidden">
        <div className="absolute inset-0">
          ...
          {/* Floating Gold & Emerald Particles — FIXED SYNTAX */}
          {[...Array(20)].map((_, i) => (
            <motion.div
              key={i}
              animate={{
        ...
        >
          ...
          >
            {t('preparingOccupancyReport') || "Preparing Your Royal Occupancy Report..."}
          </motion.p>
          ...
          <div className="w-full max-w-3xl mx-auto">
            ...
            >
              {t('calculatingRooms') || "Calculating Your Royal Rooms..."}
            </motion.p>
          </div>
        </motion.div>
      </div>
  ...
  return (
    <div className="container mx-auto p-6 bg-gray-50 dark:bg-gray-900 min-h-screen">
      <h1 className="text-3xl font-bold mb-8 text-gray-900 dark:text-white flex items-center">
        <Hotel className="mr-3" size={32} /> {t('occupancy')} {t('generateReport').split(' ')[1]}
      </h1>
      ...
      {reportData ? (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          ...
          <Card className="shadow-lg dark:bg-gray-800 dark:border-gray-700">
            <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
              <CardTitle className="text-sm font-medium text-gray-700 dark:text-gray-300">{t('occupancyRate')}</CardTitle>
          ...
          <Card className="shadow-lg dark:bg-gray-800 dark:border-gray-700">
            <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
              <CardTitle className="text-sm font-medium text-gray-700 dark:text-gray-300">{t('totalRooms')}</CardTitle>
          ...
          <Card className="shadow-lg dark:bg-gray-800 dark:border-gray-700">
            <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
              <CardTitle className="text-sm font-medium text-gray-700 dark:text-gray-300">{t('occupiedRooms')}</CardTitle>
          ...
          <Card className="shadow-lg dark:bg-gray-800 dark:border-gray-700">
            <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
              <CardTitle className="text-sm font-medium text-gray-700 dark:text-gray-300">{t('availableRooms')}</CardTitle>
        ...
        <div className="text-center text-gray-600 dark:text-gray-400 mt-10">
          {t('selectDateRangePrompt') || "Please select a date range and generate the report"}
        </div>
      )}
    </div>
  );
