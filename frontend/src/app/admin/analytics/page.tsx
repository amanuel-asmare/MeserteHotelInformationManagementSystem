...
export default function FinancialAnalytics() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        // Map backend data to translated keys if needed, but Recharts tooltip formatter handles display
      ...
      <div className="relative z-10 flex flex-col items-center">
        ...
        >
          <h3 className="text-xl font-black text-gray-800 tracking-tight flex items-center gap-2">
            ...
            {t('processingFinancials')}
          </h3>
          <p className="text-sm text-gray-400 mt-2 font-medium">{t('aggregatingData')}</p>
        </motion.div>
      </div>
    </div>
  );
  ...
  return (
    <div className="space-y-8 pb-12 ">
      ...
      >
        <div className='ml-20'>
          <h1 className="text-3xl font-black text-gray-900 ">{t('financialPerformance')}</h1>
          <p className="text-gray-500">{t('financialAnalysisDesc')}</p>
        </div>
        <div className="bg-white px-4 py-2 rounded-xl shadow-sm border border-gray-100 flex items-center gap-2">
          <PieChart size={18} className="text-amber-600" />
          <span className="font-bold text-gray-700 text-sm">{t('fiscalYear')}: {new Date().getFullYear()}</span>
        </div>
      </motion.div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }} className="bg-white p-6 rounded-3xl shadow-lg shadow-green-500/5 border border-green-100 relative overflow-hidden">
          <div className="flex justify-between items-start z-10 relative">
            <div>
              <p className="text-sm font-bold text-gray-400 uppercase tracking-wider">{t('totalIncome')}</p>
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }} className="bg-white p-6 rounded-3xl shadow-lg shadow-red-500/5 border border-red-100 relative overflow-hidden">
          <div className="flex justify-between items-start z-10 relative">
            <div>
              <p className="text-sm font-bold text-gray-400 uppercase tracking-wider">{t('totalExpenses')}</p>
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3 }} className="bg-gradient-to-br from-gray-900 to-gray-800 p-6 rounded-3xl shadow-xl text-white relative overflow-hidden">
          <div className="flex justify-between items-start z-10 relative">
            <div>
              <p className="text-sm font-bold text-gray-400 uppercase tracking-wider">{t('netProfit')}</p>
              ...
              <div className="mt-2 inline-flex items-center gap-1 bg-white/10 px-2 py-1 rounded-lg text-xs font-bold">
                ...
                <span className={Number(profitMargin) >= 0 ? "text-green-400" : "text-red-400"}>{profitMargin}% {t('margin')}</span>
              </div>
            </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        ...
        <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} transition={{ delay: 0.4 }} className="bg-white p-6 rounded-3xl shadow-sm border border-gray-100">
          <div className="mb-6">
            <h3 className="text-lg font-bold text-gray-900 flex items-center gap-2"><DollarSign size={20} className="text-green-600"/> {t('revenueStream')}</h3>
            <p className="text-xs text-gray-500">{t('revenueStreamDesc')}</p>
          </div>
            ...
            <ResponsiveContainer width="100%" height="100%">
              <AreaChart data={data}>
                ...
                <Tooltip 
                  ...
                  formatter={(value: number) => [formatCurrency(value), t('income')]}
                />
        ...
        <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} transition={{ delay: 0.5 }} className="bg-white p-6 rounded-3xl shadow-sm border border-gray-100">
          <div className="mb-6">
            <h3 className="text-lg font-bold text-gray-900 flex items-center gap-2"><TrendingDown size={20} className="text-red-600"/> {t('costAnalysis')}</h3>
            <p className="text-xs text-gray-500">{t('costAnalysisDesc')}</p>
          </div>
          <div className="h-[300px] w-full">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={data}>
                ...
                <Tooltip 
                  ...
                  contentStyle={{borderRadius: '12px', border: 'none', boxShadow: '0 4px 6px -1px rgba(0, 0, 0, 0.1)'}}
                  formatter={(value: number) => [formatCurrency(value), t('expense')]}
                />
      ...
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.6 }} className="bg-white p-8 rounded-3xl shadow-lg border border-gray-100">
        <div className="flex flex-col md:flex-row justify-between items-center mb-8">
          <div>
            <h3 className="text-2xl font-black text-gray-900">{t('profitCostComparison')}</h3>
            <p className="text-gray-500">{t('comprehensiveView')}</p>
          </div>
          <div className="flex gap-4 mt-4 md:mt-0">
             <div className="flex items-center gap-2 text-sm text-gray-600"><div className="w-3 h-3 rounded-full bg-emerald-500"></div> {t('income')}</div>
             <div className="flex items-center gap-2 text-sm text-gray-600"><div className="w-3 h-3 rounded-full bg-rose-500"></div> {t('expense')}</div>
             <div className="flex items-center gap-2 text-sm text-gray-600"><div className="w-3 h-3 rounded-full bg-amber-500"></div> {t('profit')}</div>
          </div>
        </div>
        ...
        <div className="h-[400px] w-full">
          <ResponsiveContainer width="100%" height="100%">
            <ComposedChart data={data} margin={{ top: 20, right: 20, bottom: 20, left: 20 }}>
              ...
              <Legend />
              {/* Income Bar */}
              <Bar dataKey="Income" name={t('income')} barSize={20} fill="#10B981" radius={[4, 4, 0, 0]} />
              {/* Expense Bar */}
              <Bar dataKey="Expense" name={t('expense')} barSize={20} fill="#F43F5E" radius={[4, 4, 0, 0]} />
              {/* Profit Line */}
              <Line type="monotone" dataKey="Profit" name={t('profit')} stroke="#F59E0B" strokeWidth={4} dot={{r: 4, fill: '#F59E0B', strokeWidth: 2, stroke: '#fff'}} />
            </ComposedChart>
          </ResponsiveContainer>
        </div>
      </motion.div>
