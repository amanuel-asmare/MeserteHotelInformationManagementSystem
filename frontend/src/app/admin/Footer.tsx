      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8 mb-10">
          ...
          <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }} className="space-y-4">
            ...
            <div className="flex gap-3">
              {socialLinks.map(({ icon: Icon, href, label }) => (
                <motion.a key={label} href={href} target="_blank" rel="noopener noreferrer" whileHover={{ scale: 1.2, rotate: 5 }} whileTap={{ scale: 0.9 }} className="p-2 bg-amber-100 dark:bg-amber-900/30 rounded-lg hover:bg-amber-200 dark:hover:bg-amber-900/50 transition shadow-sm" aria-label={`Follow us on ${label}`}>
          ...
          <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }} className="space-y-4">
            ...
            <div className="space-y-3 text-sm text-gray-600 dark:text-gray-400">
              <a href="tel:+251911234567" className="flex items-center gap-2 hover:text-amber-600 dark:hover:text-amber-400 transition group"><div className="w-1 h-1 bg-amber-500 rounded-full group-hover:scale-150 transition"></div> +251 911 234 567</a>
              <a href="tel:+251987654321" className="flex items-center gap-2 hover:text-amber-600 dark:hover:text-amber-400 transition group"><div className="w-1 h-1 bg-amber-500 rounded-full group-hover:scale-150 transition"></div> +251 987 654 321</a>
              <a href="mailto:info@meserethotel.com" className="flex items-center gap-2 hover:text-amber-600 dark:hover:text-amber-400 transition group"><Mail size={16} /> info@meserethotel.com</a>
              <div className="flex items-start gap-2"><MapPin size={16} className="mt-0.5 text-amber-600 dark:text-amber-400" /><p>P.O. Box 1234, Woldia, Ethiopia</p></div>
            </div>
          </motion.div>
          ...
          <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} transition={{ delay: 0.3 }} className="space-y-4">
            ...
            <ul className="space-y-2 text-sm text-gray-600 dark:text-gray-400">
              {['24/7 Room Service', 'Fine Dining Restaurant', 'Conference & Events', 'Free High-Speed WiFi', 'Laundry & Dry Cleaning', 'Business Center'].map(s => (
                <li key={s} className="flex items-center gap-2 hover:text-amber-600 dark:hover:text-amber-400 transition cursor-default group">
          ...
          <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} transition={{ delay: 0.4 }} className="space-y-4">
            ...
            <ul className="space-y-2 text-sm text-gray-600 dark:text-gray-400">
              {[
                ...
                <li key={l.label}>
                  <a href={l.href} className="hover:text-amber-600 dark:hover:text-amber-400 transition flex items-center gap-2 group">
        ...
        <div className="border-t border-gray-200 dark:border-gray-700 pt-6 flex flex-col md:flex-row justify-between items-center text-xs text-gray-500 dark:text-gray-400">
          <div className="flex flex-col sm:flex-row items-center gap-4 mb-4 md:mb-0">
            <p>© 2025 <span className="font-semibold text-amber-600 dark:text-amber-400">Meseret Hotel</span>. All rights reserved.</p>
