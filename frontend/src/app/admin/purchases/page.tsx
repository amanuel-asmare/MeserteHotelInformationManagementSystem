...
export default function PurchasesPage() {
  ...
  const handleSubmit = async (e: any) => {
    ...
    } catch (err) { alert(t('failedSavePurchase' as any)); }
  };

  const handleDelete = async (id: string, e: any) => {
    e.stopPropagation(); // Prevent opening detail modal
    if(!confirm(t('deletePurchaseConfirm' as any))) return;
    try {
      await axios.delete(`${API_URL}/api/finance/purchases/${id}`, { withCredentials: true });
      fetchPurchases();
    } catch(err) { alert(t('failedDelete' as any)); }
  };
  ...
  const getTranslatedCategory = (cat: string) => {
      // Map known categories to keys using 'as any' to avoid type errors
      if (cat === 'Food & Beverage') return t('foodBeverage' as any);
      if (cat === 'Housekeeping') return t('housekeeping' as any);
      if (cat === 'Furniture') return t('furniture' as any);
      if (cat === 'Electronics') return t('electronics' as any);
      if (cat === 'Linens') return t('linens' as any);
      if (cat === 'Maintenance Material') return t('maintenanceMaterial' as any);
      return cat;
  }
  ...
  return (
    <div className="space-y-8 pb-12">
      ...
      <div className="bg-white p-6 rounded-3xl shadow-sm border border-gray-100 flex flex-col md:flex-row justify-between items-center gap-4">
        <div>
          <h1 className="text-3xl font-black text-gray-900 tracking-tight">{t('purchaseInventory' as any)}</h1>
          <p className="text-gray-500">{t('purchaseDesc' as any)}</p>
        </div>
        <div className="flex gap-3">
          <div className="relative">
            ...
            <input 
              type="text" 
              placeholder={t('searchSupplier' as any)} 
          ...
          >
            <Plus size={20} /> {t('newPurchase' as any)}
          </button>
        </div>
      </div>
      ...
      <div className="space-y-4">
        {isLoading ? (
          ...
          <div className="text-center py-20 text-gray-400 bg-white rounded-3xl border border-dashed border-gray-200">
            ...
            <p>{t('noPurchaseRecords' as any)}</p>
          </div>
        ) : (
            ...
            <table className="w-full">
              <thead className="bg-gray-50 border-b border-gray-100">
                <tr>
                  ...
                  <th className="px-6 py-4 text-left text-xs font-bold text-gray-500 uppercase tracking-wider">{t('supplierInfo' as any)}</th>
                  <th className="px-6 py-4 text-left text-xs font-bold text-gray-500 uppercase tracking-wider">{t('itemsSummary' as any)}</th>
                  <th className="px-6 py-4 text-left text-xs font-bold text-gray-500 uppercase tracking-wider">{t('totalAmount' as any)}</th>
                  ...
                  {filteredPurchases.map((p: any) => (
                    ...
                    >
                      ...
                      <td className="px-6 py-5">
                        ...
                        <span className="text-xs text-gray-500">{p.items.length} {t('itemsPurchased' as any)}</span>
                      </td>
                      <td className="px-6 py-5">
                         <span className="block text-sm font-black text-gray-900">{formatCurrency(p.grandTotal)}</span>
                         <span className="text-[10px] text-green-600 bg-green-50 px-2 py-0.5 rounded-full font-bold uppercase tracking-wide">{t('paidVia' as any)} {p.paymentMethod}</span>
                      </td>
      ...
      {isFormOpen && (
        ...
        >
          ...
          <div className="bg-gray-900 p-6 flex justify-between items-center text-white shrink-0">
            <div>
               <h2 className="text-2xl font-bold">{t('newPurchase' as any)}</h2>
               <p className="text-gray-400 text-sm">{t('recordIncoming' as any)}</p>
            </div>
          ...
          <form onSubmit={handleSubmit}>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-6">
              <div>
                <label className="block text-xs font-bold text-gray-500 uppercase mb-2">{t('supplierName' as any)}</label>
              ...
              <div>
                <label className="block text-xs font-bold text-gray-500 uppercase mb-2">{t('invoiceRef' as any)}</label>
              ...
              <div className="flex items-end pb-3">
                 <label className="flex items-center gap-3 cursor-pointer p-3 bg-amber-50 rounded-xl w-full border border-amber-100 hover:bg-amber-100 transition">
                    ...
                    <span className="font-bold text-gray-800 text-sm">{t('includesVat' as any)}</span>
                 </label>
              </div>
            </div>

            <div className="bg-gray-50 p-6 rounded-3xl border border-gray-100 mb-6">
              <h3 className="font-bold text-gray-700 mb-4 flex items-center gap-2"><ShoppingBag size={18}/> {t('itemsList' as any)}</h3>
              {formData.items.map((item, index) => (
                <div key={index} className="grid grid-cols-1 md:grid-cols-12 gap-4 mb-3 items-end bg-white p-4 rounded-xl shadow-sm border border-gray-100">
                  <div className="md:col-span-4">
                    <label className="text-[10px] font-bold text-gray-400 uppercase">{t('itemName' as any)}</label>
                    <input required type="text" placeholder={t('itemName' as any)} className="w-full p-2 bg-transparent border-b border-gray-200 focus:border-amber-500 outline-none font-medium" 
                  ...
                  <div className="md:col-span-2">
                    <label className="text-[10px] font-bold text-gray-400 uppercase">{t('qty' as any)}</label>
                  ...
                  <div className="md:col-span-2">
                    <label className="text-[10px] font-bold text-gray-400 uppercase">{t('unitPrice' as any)}</label>
              ...
              <button type="button" onClick={addItemRow} className="mt-4 w-full py-3 rounded-xl border-2 border-dashed border-gray-300 text-gray-500 hover:border-amber-500 hover:text-amber-600 font-bold transition flex items-center justify-center gap-2">
                <Plus size={18}/> {t('addAnotherItem' as any)}
              </button>
            </div>

            <div className="flex justify-end items-center gap-8 border-t border-gray-100 pt-6">
              <div className="text-right space-y-1">
                {/* FIX: Cast 'subtotal' and 'vat' to any to avoid strict type error */}
                <p className="text-sm text-gray-500">{t('subtotal' as any)}: <span className="font-bold text-gray-800">{formatCurrency(formSubTotal)}</span></p>
                <p className="text-sm text-gray-500">{t('vat' as any)}: <span className="font-bold text-gray-800">{formatCurrency(formTax)}</span></p>
              ...
              <div className="flex gap-3">
                ...
                <button type="submit" className="px-6 py-3 rounded-xl font-bold text-white bg-green-600 hover:bg-green-700 flex items-center gap-2 shadow-lg shadow-green-200"><CheckCircle2 size={18}/> {t('savePurchase' as any)}</button>
              </div>
            </div>
          </form>
          </div>
            ...
            >
              ...
              <div className="bg-gradient-to-r from-gray-900 to-gray-800 p-8 text-white relative overflow-hidden shrink-0">
                ...
                <div className="relative z-10 flex justify-between items-start">
                  <div>
                    <h2 className="text-3xl font-black mb-1">{t('invoice' as any)}</h2>
                    <p className="text-gray-400 font-mono text-sm tracking-wider">#{selectedPurchase.referenceNo || 'NO-REF'}</p>
                  </div>
                  <div className="text-right">
                    <p className="text-gray-400 text-xs uppercase font-bold">{t('totalAmount' as any)}</p>
              ...
              <div className="p-8 overflow-y-auto">
                ...
                <div className="flex justify-between items-center mb-8 pb-6 border-b border-gray-100">
                   <div>
                      <p className="text-xs font-bold text-gray-400 uppercase mb-1">{t('supplierInfo' as any)}</p>
                  ...
                  <table className="w-full text-sm">
                    <thead>
                      <tr className="text-gray-400 border-b border-gray-200">
                        <th className="pb-3 text-left font-bold uppercase text-[10px]">{t('itemName' as any)}</th>
                        <th className="pb-3 text-center font-bold uppercase text-[10px]">{t('qty' as any)}</th>
                        <th className="pb-3 text-right font-bold uppercase text-[10px]">{t('price' as any)}</th>
                        <th className="pb-3 text-right font-bold uppercase text-[10px]">{t('total' as any)}</th>
                      </tr>
                    </thead>
                ...
                <div className="mt-8 pt-6 border-t border-gray-100 flex justify-between items-center">
                   <div className="flex items-center gap-2 text-green-600 bg-green-50 px-3 py-1 rounded-full text-xs font-bold uppercase">
                      <CheckCircle2 size={14} /> {t('statusReceived' as any)}
                   </div>
                   <button onClick={() => setSelectedPurchase(null)} className="text-gray-500 hover:text-gray-900 text-sm font-bold underline">{t('closeReceipt' as any)}</button>
                </div>
              </div>
            </motion.div>
          </div>
