        ...
        {payload.map((entry: any, index: number) => (
          <div key={index} className="flex items-center gap-2 text-sm">
            ...
            <span className="text-gray-600 capitalize">
                {entry.name === 'Paid' ? t('paid') : 
                 entry.name === 'Pending' ? t('pending') :
                 entry.name === 'Net Pay' ? t('netPay') :
                 entry.name === 'Tax & Pension' ? t('taxPension') :
                 entry.name}
                :
            </span>
            <span className="font-bold text-gray-900">
              {entry.name === 'Amount' || entry.name === 'Net Pay' || entry.name === 'Tax & Pension'
...
export default function PayrollClient() {
  ...
  const fetchPayroll = async () => {
    ...
    try {
      ...
      if (err.response?.status !== 404) {
        const msg = err.response?.data?.message || t('failedLoad');
        setError(msg);
        toast.error(msg);
      }
      setPayslips([]); 
  ...
  const handleGeneratePayroll = async () => {
    const monthName = new Date(selectedPeriod.year, selectedPeriod.month - 1).toLocaleString('default', { month: 'long' });
    if (!confirm(`${t('generateConfirm')} ${monthName} ${selectedPeriod.year}?`)) return;
  ...
  // Dynamic Date Formatting for Header
  const currentPeriodStr = new Date(selectedPeriod.year, selectedPeriod.month - 1).toLocaleDateString(language === 'am' ? 'am-ET' : 'en-US', { month: 'long', year: 'numeric' });
    ...
    <div className="min-h-screen bg-[#FDFBF7] dark:bg-gray-900 p-4 sm:p-6 lg:p-8 font-sans">
      ...
      <div className="max-w-7xl mx-auto mb-10">
        <div className="flex flex-col md:flex-row justify-between items-end gap-6">
          ...
          <div className="flex gap-4 items-center bg-white p-2 rounded-2xl shadow-sm border border-gray-200">
            ...
            >
              {[...Array(12)].map((_, i) => (
                <option key={i + 1} value={i + 1}>{new Date(0, i).toLocaleString(language === 'am' ? 'am-ET' : 'default', { month: 'short' })}</option>
              ))}
            </select>
             ...
             <motion.button
              ...
              className={`ml-2 px-6 py-2.5 rounded-xl font-bold text-sm shadow-lg transition-all flex items-center gap-2 ${
                isGenerating
                  ...
                  : 'bg-gradient-to-r from-amber-50 to-orange-600 text-white hover:shadow-orange-500/30'
              }`}
            >
      ...
      <div className="max-w-7xl mx-auto grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
        ...
        <StatCard 
          ...
          subValue={t('completedTransactions')}
          icon={<Users size={24} />} 
            ...
            <div className="flex flex-col lg:flex-row gap-8 items-center">
              ...
              <div className="lg:w-1/3 space-y-4">
                <div className="inline-flex items-center gap-2 px-4 py-2 bg-amber-50 text-amber-700 rounded-full text-sm font-bold">
                  ...
                  {activeView === 'overview' ? t('financialOverview') : activeView === 'paid' ? t('paymentDistribution') : t('pendingPayments')}
                </div>
                <h2 className="text-3xl font-black text-gray-800">
                  {activeView === 'overview' && t('monthlyExpenditure')}
                  {activeView === 'paid' && t('completionStatus')}
                  {activeView === 'pending' && t('outstandingPayments')}
                </h2>
                <p className="text-gray-500 leading-relaxed">
                  {activeView === 'overview' && t('expenditureDesc')}
                  {activeView === 'paid' && t('completionDesc')}
                  {activeView === 'pending' && t('outstandingDesc')}
                </p>
              </div>
                ...
                <ResponsiveContainer width="100%" height="100%">
                  {activeView === 'overview' ? (
                    <BarChart data={departmentData} margin={{ top: 20, right: 30, left: 20, bottom: 5 }}>
                      ...
                      <Bar dataKey="value" name={t("amount")} radius={[8, 8, 0, 0]}>
                    ...
                    <PieChart>
                      ...
                      <Legend 
                        ...
                        formatter={(val) => val === 'Paid' ? t('paid') : t('pending')} // Translate Legend
                      />
                    </PieChart>
                  ) : (
                     <BarChart data={salaryComponentsData} layout="vertical" margin={{ top: 20, right: 30, left: 20, bottom: 5 }}>
                      ...
                      <YAxis dataKey="name" type="category" width={100} axisLine={false} tickLine={false} tickFormatter={(val) => val === 'Net Pay' ? t('netPay') : t('taxPension')} />
                      <RechartsTooltip content={<CustomTooltip />} />
      ...
      <div className="max-w-7xl mx-auto">
        {loading ? (
          <div className="flex flex-col items-center justify-center py-20 bg-white rounded-3xl shadow-sm border border-gray-100">
             ...
             <p className="mt-4 text-amber-800 font-medium">{t('loadingPayroll')}</p>
          </div>
        ) : displayData.length === 0 ? (
          <div className="text-center py-24 bg-white rounded-3xl shadow-xl border border-dashed border-gray-300">
            <Users className="w-20 h-20 text-gray-300 mx-auto mb-4" />
            <p className="text-xl font-bold text-gray-600">{t('noActiveStaff')}</p>
            <p className="text-gray-400">{t('addEmployeesFirst')}</p>
          </div>
        ) : (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
            {payslips.length === 0 && (
              <div className="mb-8 p-6 bg-amber-50 border border-amber-200 rounded-2xl flex items-center gap-4">
                ...
                <div>
                  <h3 className="text-lg font-bold text-amber-900">{t('payrollNotGenerated')}</h3>
                  <p className="text-amber-700">{t('payrollPreviewNote')}</p>
                </div>
              </div>
            )}
