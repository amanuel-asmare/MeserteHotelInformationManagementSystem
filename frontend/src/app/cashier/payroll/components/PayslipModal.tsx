...
export default function PayslipModal({
  ...
  const handleSave = async (data: any) => {
    if (!onUpdate || !payslip?._id) {
      // FIX: Cast 'cannotSave' to any
      toast.error(t('cannotSave' as any) || 'Cannot save: no update handler available');
      return;
    }
    const updates = {
    ...
    if (success) {
      // FIX: Cast 'payslipUpdated' to any (assuming it might also be missing)
      toast.success(t('payslipUpdated' as any) || 'Payslip updated successfully!');
      setIsEditing(false);
      reset(data);
    }
  };
