...
import { CalendarIcon, UserIcon, EditIcon, HistoryIcon, PlusCircleIcon, Info } from 'lucide-react';
...
export default function ManageAttendancePage() {
  ...
  const fetchAllStaff = async () => {
    try {
      ...
      toast.error(t('failedLoadStaff' as any) || 'Failed to fetch staff list.');
    }
  };
  ...
  const fetchAttendanceForDate = async (date: Date) => {
    ...
    try {
      ...
      toast.error(t('failedLoadAttendance' as any) || 'Failed to load attendance for this date.');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleEditClick = (entry: StaffAttendanceEntry) => {
    if (!entry.attendance) {
      // FIX: Changed toast.info to toast() with an icon because .info() doesn't exist in react-hot-toast
      toast(t('noRecordUseCreate' as any) || 'No existing record. Use "Mark Attendance" to add a new entry.', {
        icon: 'ℹ️',
        style: {
          borderRadius: '10px',
          background: '#333',
  ...
  const handleUpdateAttendance = async () => {
    ...
    try {
      ...
      toast.success(t('statusUpdated' as any) || 'Attendance updated successfully!');
      setIsEditModalOpen(false);
      fetchAttendanceForDate(selectedDate);
    } catch (error: any) {
      toast.error(error.response?.data?.message || t('failedUpdate' as any) || 'Failed to update attendance.');
    }
  };
  ...
  const handleViewHistoryClick = async (staff: User) => {
    ...
    try {
      ...
      toast.error(error.response?.data?.message || t('failedLoadHistory' as any) || 'Failed to load staff attendance history.');
      setStaffHistory([]);
    }
  };
  ...
  const handleCreateAttendance = async () => {
    if (!createUserId || !createDate || !createStatus) {
      toast.error(t('requiredFields' as any) || 'Staff, date, and status are required to create attendance.');
      return;
    }
    ...
    try {
      ...
      toast.success(t('attendanceCreated' as any) || 'Attendance created successfully!');
      ...
      toast.error(error.response?.data?.message || t('failedCreate' as any) || 'Failed to create attendance.');
    }
  };
    ...
    <div className="container mx-auto p-6 md:p-8">
      <Card className="shadow-lg">
        <CardHeader>
          <CardTitle className="text-3xl font-bold flex items-center gap-3">
            ...
            {t('manageStaffAttendance' as any) || "Manage Staff Attendance"}
          </CardTitle>
          <CardDescription className="text-gray-600 dark:text-gray-400 mt-2">
            {t('manageStaffDesc' as any) || "View and manage daily attendance for all staff members (Receptionists, Cashiers)."}
          </CardDescription>
        </CardHeader>
        <CardContent className="space-y-6">
          <div className="flex flex-col md:flex-row items-center justify-between gap-4 p-4 bg-gray-50 dark:bg-gray-800 rounded-lg shadow-inner">
            <div className="flex items-center gap-3">
              ...
              <DatePicker
                date={selectedDate}
                // FIX: Wrapper to handle undefined dates
                setDate={(date) => {
            ...
            <Button onClick={() => setIsCreateModalOpen(true)} className="flex items-center gap-2 bg-blue-600 hover:bg-blue-700 text-white">
              <PlusCircleIcon size={20} /> {t('markAttendance' as any) || "Mark Attendance"}
            </Button>
          </div>
              ...
              <TableHeader className="bg-gray-100 dark:bg-gray-800">
                <TableRow>
                  <TableHead className="px-6 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-300 uppercase tracking-wider">
                    {t('staffMember' as any) || "Staff Member"}
                  </TableHead>
                  <TableHead className="px-6 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-300 uppercase tracking-wider">
                    {t('role') || "Role"}
                  </TableHead>
                  <TableHead className="px-6 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-300 uppercase tracking-wider">
                    {t('checkIn' as any) || "Check-in"}
                  </TableHead>
                  <TableHead className="px-6 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-300 uppercase tracking-wider">
                    {t('checkOut' as any) || "Check-out"}
                  </TableHead>
                  <TableHead className="px-6 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-300 uppercase tracking-wider">
                    {t('status') || "Status"}
                  </TableHead>
                ...
                {loading ? (
                  <TableRow>
                    <TableCell colSpan={6} className="text-center py-8 text-gray-500 dark:text-gray-400">
                      {t('loadingAttendance' as any) || "Loading attendance data..."}
                    </TableCell>
                  </TableRow>
                ) : attendanceList.length === 0 ? (
                  <TableRow>
                    <TableCell colSpan={6} className="text-center py-8 text-gray-500 dark:text-gray-400">
                      {t('noRecordsForDate' as any) || "No staff attendance records found for this date."}
                    </TableCell>
                  </TableRow>
                ) : (
                    ...
                    <TableRow key={entry.staff._id}>
                      ...
                      <TableCell className="px-6 py-4 whitespace-nowrap text-sm">
                        <span className={`px-2 inline-flex text-xs leading-5 font-semibold rounded-full capitalize ${getStatusColor(entry.attendance?.status || 'absent')}`}>
                          {/* FIX: Cast status to any to bypass strict translation key check */}
                          {t((entry.attendance?.status || 'absent') as any) || (entry.attendance?.status || 'N/A (Absent)')}
                        </span>
                      </TableCell>
        ...
        <DialogContent>
          <DialogHeader>
            <DialogTitle className="text-2xl font-bold flex items-center gap-2">
              <EditIcon size={24} /> {t('editAttendance' as any) || "Edit Attendance"}
            </DialogTitle>
            <DialogDescription className="text-gray-600 dark:text-gray-400">
              {t('adjustFor' as any) || "Adjust attendance details for"} {currentAttendance?.user?.firstName} {currentAttendance?.user?.lastName} {t('on' as any) || "on"} {formatDateForDisplay(currentAttendance?.date || '')}.
            </DialogDescription>
          </DialogHeader>
          {currentAttendance && (
            <div className="space-y-4 py-4">
              <div>
                <Label htmlFor="editCheckIn" className="mb-1 block text-sm font-medium">{t('checkInTime' as any) || "Check-in Time"}</Label>
                <input id="editCheckIn" type="time" value={editCheckIn} onChange={(e) => setEditCheckIn(e.target.value)} />
              </div>
              <div>
                <Label htmlFor="editCheckOut" className="mb-1 block text-sm font-medium">{t('checkOutTime' as any) || "Check-out Time"}</Label>
              ...
              <div>
                ...
                <Select value={editStatus} onValueChange={(value: AttendanceStatus) => setEditStatus(value)}>
                  ...
                  <SelectContent>
                    ...
                    <SelectItem value="leave">{t('onLeave' as any) || "On Leave"}</SelectItem>
              ...
              <div>
                <Label htmlFor="editNotes" className="mb-1 block text-sm font-medium">{t('notes' as any) || "Notes"}</Label>
                <Textarea
                  id="editNotes"
                  value={editNotes}
                  onChange={(e) => setEditNotes(e.target.value)}
                  placeholder={t('addNotes' as any) || "Add notes about this attendance record..."}
                />
              </div>
            </div>
          )}
      ...
      <Dialog open={isCreateModalOpen} onOpenChange={setIsCreateModalOpen}>
        <DialogContent className="max-w-md md:max-w-lg lg:max-w-xl max-h-[90vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle className="text-2xl font-bold flex items-center gap-2">
              <PlusCircleIcon size={24} /> {t('markAttendance' as any) || "Mark Staff Attendance"}
            </DialogTitle>
            <DialogDescription className="text-gray-600 dark:text-gray-400">
              {t('manuallyMark' as any) || "Manually mark attendance for a staff member, especially for absences or leaves."}
            </DialogDescription>
          </DialogHeader>
          <div className="space-y-4 py-4 pr-2">
            <div>
              <Label htmlFor="createStaff" className="mb-1 block text-sm font-medium">{t('staffMember' as any) || "Staff Member"}</Label>
              <Select value={createUserId} onValueChange={setCreateUserId}>
                <SelectTrigger className="w-full">
                  <SelectValue placeholder={t('selectStaff' as any) || "Select Staff"} />
                </SelectTrigger>
                <SelectContent>
                  {allStaff.map(staff => (
            ...
            <div>
              ...
              <DatePicker
                date={createDate}
                // FIX: Wrapper to handle undefined dates here as well
                setDate={(date) => {
            ...
            {(createStatus === 'present' || createStatus === 'half-day') && (
              <>
                <div>
                  <Label htmlFor="createCheckIn" className="mb-1 block text-sm font-medium">{t('checkInTimeOptional' as any) || "Check-in Time (Optional)"}</Label>
                  <input id="createCheckIn" type="time" value={createCheckIn} onChange={(e) => setCreateCheckIn(e.target.value)} />
                </div>
                <div>
                  <Label htmlFor="createCheckOut" className="mb-1 block text-sm font-medium">{t('checkOutTimeOptional' as any) || "Check-out Time (Optional)"}</Label>
                  <input id="createCheckOut" type="time" value={createCheckOut} onChange={(e) => setCreateCheckOut(e.target.value)} />
                </div>
              </>
            )}
            <div>
              <Label htmlFor="createNotes" className="mb-1 block text-sm font-medium">{t('notesOptional' as any) || "Notes (Optional)"}</Label>
              <Textarea
                id="createNotes"
                value={createNotes}
                onChange={(e) => setCreateNotes(e.target.value)}
                placeholder={t('addNotesReason' as any) || "Add notes (e.g., reason for absence/leave)..."}
              />
            </div>
          </div>
          <DialogFooter className="mt-6 flex justify-end gap-2">
            <Button variant="outline" onClick={() => setIsCreateModalOpen(false)}>{t('cancel') || "Cancel"}</Button>
            <Button onClick={handleCreateAttendance}>{t('createAttendance' as any) || "Create Attendance"}</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
      ...
      <Dialog open={isHistoryModalOpen} onOpenChange={setIsHistoryModalOpen}>
        <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle className="text-2xl font-bold flex items-center gap-2">
              <HistoryIcon size={24} /> {t('attendanceHistoryFor' as any) || "Attendance History for"} {currentStaffForHistory?.firstName} {currentStaffForHistory?.lastName}
            </DialogTitle>
            <DialogDescription className="text-gray-600 dark:text-gray-400">
              {t('completeRecord' as any) || "A complete record of past attendance."}
            </DialogDescription>
          </DialogHeader>
          <div className="py-4 max-h-[60vh] overflow-y-auto pr-2">
            {staffHistory.length === 0 ? (
              <p className="text-center text-gray-500 dark:text-gray-400">{t('noHistory' as any) || "No attendance history available."}</p>
            ) : (
              <Table>
                <TableHeader>
                  <TableRow>
                    <TableHead>{t('date') || "Date"}</TableHead>
                    <TableHead>{t('checkIn' as any) || "Check-in"}</TableHead>
                    <TableHead>{t('checkOut' as any) || "Check-out"}</TableHead>
                    <TableHead>{t('status') || "Status"}</TableHead>
                    <TableHead>{t('notes' as any) || "Notes"}</TableHead>
                    <TableHead>{t('markedBy' as any) || "Marked By"}</TableHead>
                  </TableRow>
                </TableHeader>
                <TableBody>
                    ...
                    <TableRow key={record._id}>
                      ...
                      <TableCell>
                        <span className={`px-2 inline-flex text-xs leading-5 font-semibold rounded-full capitalize ${getStatusColor(record.status)}`}>
                          {/* FIX: Cast record status to any */}
                          {t(record.status as any) || record.status}
                        </span>
                      </TableCell>
                      <TableCell className="max-w-[200px] truncate">{record.notes || '-'}</TableCell>
                      <TableCell>{record.markedBy ? `${record.markedBy.firstName} ${record.markedBy.lastName}` : t('self' as any) || 'Self'}</TableCell>
                    </TableRow>
                  ))}
                </TableBody>
              </Table>
...
export default function ManageAttendancePage() {
  ...
  const handleUpdateAttendance = async () => {
    ...
    try {
      await axios.put(`${API_URL}/api/attendance/manager/${currentAttendance._id}`, updatePayload, { withCredentials: true });
      toast.success(t('attendanceUpdated' as any) || 'Attendance updated successfully!');
      setIsEditModalOpen(false);
      fetchAttendanceForDate(selectedDate);
    } catch (error: any) {
