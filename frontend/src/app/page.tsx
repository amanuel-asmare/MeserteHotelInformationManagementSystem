...
import { 
  ...
  Smartphone, MapPin, Phone, Mail, Clock, Facebook, Instagram, Twitter, 
  Linkedin, ArrowRight, Hotel
} from 'lucide-react';
import { QRCodeSVG } from 'qrcode.react'; 
import { AnimatePresence, motion } from 'framer-motion';
import { useState, useEffect } from 'react';
...
const QRModal = ({ onClose, url, t }: { onClose: () => void; url: string; t: any }) => (
  <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm" onClick={onClose}>
    <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.9, opacity: 0 }} className="relative w-full max-w-sm bg-white rounded-3xl p-8 text-center shadow-2xl" onClick={(e) => e.stopPropagation()}>
      <button onClick={onClose} className="absolute top-4 right-4 p-2 text-gray-400 hover:text-red-500 transition"><X size={24} /></button>
      <div className="mb-6 flex justify-center"><div className="p-4 bg-amber-100 rounded-full text-amber-600"><ScanLine size={40} /></div></div>
      ...
      <div className="flex justify-center p-4 bg-white border-2 border-dashed border-gray-200 rounded-2xl mb-6"><QRCodeSVG value={url} size={220} level={"H"} includeMargin={true}/></div>
      <div className="flex items-center justify-center gap-2 text-xs text-amber-600 font-medium bg-amber-50 py-2 rounded-lg"><Smartphone size={16} /><span>{t('worksOnMobile' as any)}</span></div>
    </motion.div>
  </div>
);
...
export default function Home() {
  ...
  useEffect(() => {
    if (typeof window !== 'undefined') { setMenuUrl(`${window.location.origin}/customer/menu`); }
  }, []);

  const handleSearch = () => {
    if (!checkIn || !checkOut) { alert(t('selectDatesGuests')); return; }
    window.location.href = `/rooms?checkIn=${checkIn}&checkOut=${checkOut}&guests=${guests}`;
  };
  ...
  return (
    <>
      <section className="relative h-screen w-full overflow-hidden">
        <Image src="/hotel-hero.jpg" alt="Meseret Hotel" fill className="object-cover brightness-75" priority />
        <div className="absolute inset-0 bg-gradient-to-t from-black/80 via-black/30 to-transparent" />
        <div className="relative flex h-full flex-col items-center justify-center px-6 text-center text-white">
          <motion.h1 initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="mb-4 text-4xl font-bold tracking-tight sm:text-5xl md:text-7xl">
            {t('welcomeToMeseret'as any)} <span className="text-amber-400">{t('meseretHotel' as any)}</span>
          </motion.h1>
          <motion.p initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }} className="mb-8 max-w-2xl text-lg sm:text-xl font-light italic">{t('experienceLuxury' as any)}</motion.p>
          <button onClick={() => setShowQR(true)} className="mb-10 flex items-center gap-3 bg-amber-500 hover:bg-amber-600 text-white px-10 py-5 rounded-full font-bold text-lg shadow-2xl transition-all transform hover:scale-105"><ScanLine size={24} />{t('scanMenuQR' as any)}</button>

          <div className="w-full max-w-5xl rounded-2xl bg-white p-5 shadow-2xl">
            <div className="grid grid-cols-1 gap-4 md:grid-cols-4">
              <div className="flex flex-col text-left"><label className="text-[10px] font-bold text-gray-400 uppercase ml-2 mb-1">{t('checkInLabel'as any)}</label><div className="flex items-center gap-2 rounded-xl bg-gray-50 p-3"><Calendar className="text-amber-600" size={20} /><input type="date" value={checkIn} onChange={e => setCheckIn(e.target.value)} className="w-full bg-transparent text-black outline-none text-sm font-medium"/></div></div>
              <div className="flex flex-col text-left"><label className="text-[10px] font-bold text-gray-400 uppercase ml-2 mb-1">{t('checkOutLabel' as any)}</label><div className="flex items-center gap-2 rounded-xl bg-gray-50 p-3"><Calendar className="text-amber-600" size={20} /><input type="date" value={checkOut} min={checkIn} onChange={e => setCheckOut(e.target.value)} className="w-full bg-transparent text-black outline-none text-sm font-medium"/></div></div>
              <div className="flex flex-col text-left"><label className="text-[10px] font-bold text-gray-400 uppercase ml-2 mb-1">{t('guestsLabel' as any) || 'Guests'}</label><div className="flex items-center gap-2 rounded-xl bg-gray-50 p-3"><Users className="text-amber-600" size={20} /><select value={guests} onChange={e => setGuests(e.target.value)} className="w-full bg-transparent text-black outline-none text-sm font-medium">{[1, 2, 3, 4, 5, 6].map(n => <option key={n} value={n}>{n} {n > 1 ? t('guestsPlural') : t('guestSingle')}</option>)}</select></div></div>
              <div className="flex flex-col justify-end"><button onClick={handleSearch} className="h-[52px] flex items-center justify-center gap-2 rounded-xl bg-amber-600 px-6 py-3 font-bold text-white shadow-lg hover:bg-amber-700 transition-all"><Search size={20} /> {t('searchRoomsButton' as any)}</button></div>
            </div>
          </div>
        </div>
      </section>
      ...
      <section className="bg-gray-50 dark:bg-gray-950 py-24 px-6">
        <div className="container mx-auto">
          <h2 className="text-3xl font-black text-center mb-16 dark:text-white">{t('whyChooseMeseret' as any)}</h2>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8">
            {[
              ...
              <div key={i} className="flex flex-col items-center bg-white dark:bg-gray-800 p-8 rounded-3xl shadow-xl text-center border border-gray-100 dark:border-gray-700 hover:y-[-10px] transition-all">
                <f.icon className="mb-6 text-amber-600" size={40} />
                <h3 className="mb-3 text-xl font-bold dark:text-white">{t(f.title as any)}</h3>
                <p className="text-sm text-gray-600 dark:text-gray-400">{t(f.desc as any)}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      {/* CTA SECTION */}
      <section className="relative py-24 bg-amber-600 overflow-hidden px-6 text-center text-white">
        <div className="absolute top-0 right-0 p-24 opacity-10"><Hotel size={300} /></div>
        <h2 className="mb-6 text-4xl font-black">{t('readyToBook' as any)}</h2>
        <p className="mb-10 text-xl font-light opacity-90">{t('joinHappyGuests' as any)}</p>
        <button onClick={() => setShowRegister(true)} className="rounded-full bg-white text-amber-600 px-10 py-4 font-bold text-lg shadow-2xl hover:bg-gray-100 transition-all">{t('contactUsButton' as any) || 'Start Reservation'}</button>
      </section>

      {/* ENHANCED FOOTER */}
      <footer className="bg-gray-950 text-gray-300 pt-20 pb-10 border-t border-white/5 px-6">
        <div className="container mx-auto grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-12 mb-16">
          <div className="space-y-6">
            ...
            <p className="text-sm leading-relaxed text-gray-400">{t('footerBio' as any) || 'Experience the heights of luxury and the warmth of Ethiopian hospitality at Meseret Hotel.'}</p>
            <div className="flex gap-4">
              <a href="#" className="p-3 bg-white/5 hover:bg-amber-600 rounded-xl transition-all"><Facebook size={20} /></a>
              <a href="#" className="p-3 bg-white/5 hover:bg-amber-600 rounded-xl transition-all"><Instagram size={20} /></a>
              <a href="#" className="p-3 bg-white/5 hover:bg-amber-600 rounded-xl transition-all"><Twitter size={20} /></a>
            </div>
          </div>
          <div>
            <h3 className="text-white font-bold text-lg mb-6">{t('quickLinks' as any) || 'Quick Links'}</h3>
            <ul className="space-y-4 text-sm">
              <li><a href="#" className="hover:text-amber-500 transition flex items-center gap-2 group"><ArrowRight size={14}/>{t('rooms' as any)}</a></li>
              <li><a href="#" className="hover:text-amber-500 transition flex items-center gap-2 group"><ArrowRight size={14}/>{t('dining' as any)}</a></li>
              <li><a href="#" className="hover:text-amber-500 transition flex items-center gap-2 group"><ArrowRight size={14}/>{t('contact' as any) || 'Contact'}</a></li>
            </ul>
          </div>
          <div>
          ...
          <div>
            <h3 className="text-white font-bold text-lg mb-6">{t('contactUs' as any) || 'Reach Us'}</h3>
            <ul className="space-y-6 text-sm">
              <li className="flex items-start gap-4"><MapPin className="text-amber-500" size={20} /><span>{t('hotelAddress' as any) || 'Woldia, Ethiopia'}</span></li>
              <li className="flex items-center gap-4"><Phone className="text-amber-500" size={20} /><span>+251 911 000 000</span></li>
              <li className="flex items-center gap-4"><Mail className="text-amber-500" size={20} /><span>contact@meserethotel.com</span></li>
            </ul>
          </div>
        </div>
        <div className="container mx-auto pt-8 border-t border-white/5 flex flex-col md:flex-row justify-between items-center gap-4 text-xs font-medium uppercase tracking-widest text-gray-500">
