...
import {
  ...
  Image as ImageIcon, X, Bath, CreditCard, Clock, Bell, History, ListChecks // Added Clock, Bell, History, ListChecks
} from 'lucide-react';
import axios from 'axios';
import ImageCarousel from '../../../../components/ui/ImageCarousel';
import { useAuth } from '../../../../context/AuthContext';
import { format, isPast, parseISO } from 'date-fns'; // Added isPast, parseISO

// Make sure to import 'useRouter' and 'useSearchParams' from next/navigation
...
interface Booking {
  _id: string;
  room: Room;
  user: { // Add user details for consistency, even if not fully used here
    _id: string;
    firstName: string;
    lastName: string;
    email: string;
  ...
  paymentStatus: 'pending' | 'completed' | 'failed' | 'refunded'; // Added 'refunded'
  guests: number;
  paymentId?: string;
  createdAt: string; // Added for sorting/display
}
...
export default function CustomerBookingClient() {
  ...
  const [bookingStatusTab, setBookingStatusTab] = useState<'all' | 'pending' | 'confirmed' | 'cancelled' | 'completed'>('all'); // New: booking status tab
  const [showAvailableOnly, setShowAvailableOnly] = useState(true);
  const [cancelConfirmation, setCancelConfirmation] = useState<string | null>(null);
  const [notificationCount, setNotificationCount] = useState(0); // New: for notification bell
  ...
  /* ------------------------------------------------------------------ */
  /*  ✅ NEW: EFFECT TO HANDLE PAYMENT VERIFICATION                    */
  /* ------------------------------------------------------------------ */
  useEffect(() => {
    const tx_ref = searchParams.get('verify_tx_ref');

    // If a transaction reference is found in the URL...
    if (tx_ref) {
      const verify = async () => {
        try {
          // ...call our new backend endpoint to verify it.
          const res = await axios.post(`${API_BASE}/api/bookings/verify-from-client`, 
            { tx_ref },
            { withCredentials: true }
          );
          setSuccess(res.data.message || 'Payment confirmed!'); // Show success message
          fetchBookings(); // Refresh the list of bookings to show the "completed" status
        } catch (error) {
          console.error("Failed to verify payment from client:", error);
          alert('There was an issue confirming your payment. Please contact support.');
        } finally {
          // Clean the URL by removing the query parameter so this doesn't run again on refresh
  ...
  // Effect to calculate notification count
  useEffect(() => {
  ...
  const handleCancelBooking = async (bookingId: string) => {
    try {
      ...
      fetchBookings(); // Refresh customer bookings
      fetchRooms();   // Room availability might have changed
  ...
  const getImageUrl = (image: string) => {
    ...
    return image; // Assuming backend already provides full URLs
  };
    ...
    <>
        ...
        <div className="flex gap-2 relative">
          ...
          {notificationCount > 0 && (
            <motion.div
              ...
              title={`${notificationCount} booking(s) passed checkout!`}
            >
              {notificationCount}
            </motion.div>
          )}
            ...
            filteredRooms.map(room => (
              ...
              >
                ...
                <button
                  ...
                  disabled={!room.availability || room.status !== 'clean'} // Disable if not available or not clean
                >
      ...
      {tab === 'bookings' && (
        <>
          ...
          <div className="mb-6 flex gap-4 border-b border-gray-200 overflow-x-auto">
            <button
              className={`pb-3 px-4 flex items-center gap-2 font-medium text-lg whitespace-nowrap ${bookingStatusTab === 'all' ? 'border-b-2 border-amber-600 text-amber-600' : 'text-gray-500 hover:text-gray-700'}`}
              onClick={() => setBookingStatusTab('all')}
            >
              <ListChecks size={20} /> All Bookings
            </button>
            <button
              className={`pb-3 px-4 flex items-center gap-2 font-medium text-lg whitespace-nowrap ${bookingStatusTab === 'pending' ? 'border-b-2 border-amber-600 text-amber-600' : 'text-gray-500 hover:text-gray-700'}`}
              onClick={() => setBookingStatusTab('pending')}
            >
              <Clock size={20} /> Pending
            </button>
            <button
              className={`pb-3 px-4 flex items-center gap-2 font-medium text-lg whitespace-nowrap ${bookingStatusTab === 'confirmed' ? 'border-b-2 border-amber-600 text-amber-600' : 'text-gray-500 hover:text-gray-700'}`}
              onClick={() => setBookingStatusTab('confirmed')}
            >
              <CheckCircle size={20} /> Confirmed
            </button>
            <button
              className={`pb-3 px-4 flex items-center gap-2 font-medium text-lg whitespace-nowrap ${bookingStatusTab === 'completed' ? 'border-b-2 border-amber-600 text-amber-600' : 'text-gray-500 hover:text-gray-700'}`}
              onClick={() => setBookingStatusTab('completed')}
            >
              <History size={20} /> Completed
            </button>
            <button
              className={`pb-3 px-4 flex items-center gap-2 font-medium text-lg whitespace-nowrap ${bookingStatusTab === 'cancelled' ? 'border-b-2 border-amber-600 text-amber-600' : 'text-gray-500 hover:text-gray-700'}`}
              onClick={() => setBookingStatusTab('cancelled')}
            >
              <X size={20} /> Cancelled
            </button>
          </div>
          ...
          <div className="space-y-4">
            {filteredBookings.length > 0 ? (
              filteredBookings.map(booking => {
                ...
                return (
                  <motion.div
                    ...
                    initial={{ opacity: 0, y: 20 }}
                    animate={{ opacity: 1, y: 0 }}
                    className="bg-white p-6 rounded-xl shadow-sm border border-gray-200 flex flex-col sm:flex-row gap-4 relative"
                  >
                    {shouldShowCheckoutWarning && (
                      <div className="absolute top-3 right-3 bg-red-100 text-red-800 px-3 py-1 rounded-full text-sm font-medium flex items-center gap-1">
                    ...
                    <div className="flex-1">
                      ...
                      <div className="grid grid-cols-2 gap-2 text-sm text-gray-600 mt-2">
                        ...
                        <p><CheckCircle size={16} className="inline mr-1" /> Status: <span className={`font-medium ${
                          booking.status === 'confirmed' ? 'text-green-600' :
                          booking.status === 'pending' ? 'text-yellow-600' :
                          booking.status === 'cancelled' ? 'text-red-600' :
                          'text-gray-600'
                        }`}>{booking.status.charAt(0).toUpperCase() + booking.status.slice(1)}</span></p>
                        <p><CreditCard size={16} className="inline mr-1" /> Payment: <span className={`font-medium ${
                          booking.paymentStatus === 'completed' ? 'text-green-600' :
                          booking.paymentStatus === 'pending' ? 'text-orange-600' :
                    ...
                    {(booking.status === 'pending' || booking.status === 'confirmed') && (
                      <button
                        ...
                        className="py-2 px-4 bg-red-600 text-white rounded-xl hover:bg-red-700 transition self-center sm:self-auto"
                      >
                        Cancel Booking
                      </button>
                    )}
              ...
              <div className="text-center py-12 text-gray-500">
                <Bed size={48} className="mx-auto mb-3 text-gray-300" />
                <p>No {bookingStatusTab === 'all' ? '' : bookingStatusTab} bookings found.</p>
              </div>
            )}
          </div>
        </>
      ...
      {showBookingModal && (
        <motion.div
          ...
          exit={{ opacity: 0 }}
          className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4"
          onClick={() => setShowBookingModal(null)} // Close on overlay click
        >
          <motion.div
          ...
          >
            ...
            <div className="grid grid-cols-2 gap-3 mb-6">
              ...
              <div className="relative col-span-2"> {/* Made guests input full width */}
            ...
            <div className="flex gap-3 mt-6">
              ...
              <button
                onClick={() => {
                  ...
                  // setShowBookingModal(null); // Keep modal open until redirection for payment
                }}
