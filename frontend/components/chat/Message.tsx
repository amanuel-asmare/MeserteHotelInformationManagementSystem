...
// 1. Define Interface for Message Props (to be consistent and safe)
interface User {
    _id: string;
    firstName: string;
    lastName: string;
...
// 2. Add type annotation for mimeType
...
const Message = ({ message, isCurrentUser, onEdit, onDelete, onReply }: MessageProps) => {
    ...
    const handleEditSubmit = (e: React.FormEvent) => {
        e.preventDefault();
        if (message.message && editedText.trim() !== message.message) {
    ...
    const senderImage = message.sender.profileImage?.startsWith('http')
        ? message.sender.profileImage
        : `${API_BASE}${message.sender.profileImage}`;
...
export default Message;/*'use client';
