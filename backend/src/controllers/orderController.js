...
exports.getAllOrders = async(req, res) => {
    try {
        ...
        const formatted = orders.map(order => {
            ...
            // --- FIX START: Ensure consistent structure for Manager ---
            return {
                ...o,
                customer: {
                    ...o.customer,
                    // Ensure these exist even if null in DB
                    roomNumber: o.customer.roomNumber || null,
                    tableNumber: o.customer.tableNumber || null,
                    deliveryAddress: o.customer.deliveryAddress || null
                },
                // Explicitly send orderType (it might be missing in old records)
                orderType: o.orderType || (o.customer.roomNumber ? 'room' : o.customer.tableNumber ? 'table' : 'delivery'),
...
// ──────────────────────────────────────────────────────────────
// CREATE CHAPA ORDER (Payment) - FIXED FOR DELIVERY
// ──────────────────────────────────────────────────────────────
exports.createChapaOrder = async(req, res) => {
    try {
        const {
            ...
            customerName,
            email,
            phone, // This comes from frontend logic
            roomNumber,
            tableNumber,
            orderType,
            deliveryAddress
        ...
        // --- VALIDATION LOGIC ---
        const type = orderType || 'room';

        if (type === 'room' && !roomNumber) {
            return res.status(400).json({ message: 'Room number is required for room service.' });
        }
        if (type === 'table' && !tableNumber) {
            return res.status(400).json({ message: 'Table number is required for table service.' });
        }
        if (type === 'delivery') {
            if (!phone) return res.status(400).json({ message: 'Phone number is required for delivery.' });
            if (!deliveryAddress || !deliveryAddress.street || !deliveryAddress.landmark) {
                return res.status(400).json({ message: 'Street name and Landmark are required for delivery.' });
            }
        }
        ...
        calcTotal += deliveryFee;

        // Verify total (Allow small float difference)
        if (Math.abs(calcTotal - totalAmount) > 1.00) {
            return res.status(400).json({ message: `Total mismatch. Server calculated: ${calcTotal}, Client sent: ${totalAmount}` });
        }
        ...
        const order = await Order.create({
            ...
            customer: {
                ...
                phone: phone, // Explicitly use the phone sent from frontend
                roomNumber: type === 'room' ? roomNumber : undefined,
                tableNumber: type === 'table' ? tableNumber : undefined,
        ...
        // Ensure strictly digits, remove leading 0, prepend 251
        let phoneStr = (phone || '').toString().replace(/\D/g, '');

        // Handle Ethiopian formats
        if (phoneStr.startsWith('0')) {
            phoneStr = '251' + phoneStr.substring(1);
        } else if (phoneStr.startsWith('9') && phoneStr.length === 9) {
            phoneStr = '251' + phoneStr;
        } else if (phoneStr.length === 10 && phoneStr.startsWith('09')) {
            phoneStr = '251' + phoneStr.substring(1);
        }

        // Fallback for Chapa testing if phone is invalid/short
        if (phoneStr.length < 10) {
            phoneStr = '251911234567';
        }

        // --- FIX FOR TITLE ERROR ---
        // Chapa titles must be < 16 chars and no special symbols like ()
        let cleanTitle;
        if (type === 'delivery') {
            cleanTitle = `Del-${orderNumber}`; // "Del-ORD-0001" (approx 12 chars)
        } else {
            cleanTitle = `Ord-${orderNumber}`; // "Ord-ORD-0001"
        }
        ...
        const chapaPayload = {
            ...
            customization: {
                title: cleanTitle, // FIXED TITLE
                description: type === 'delivery' ? 'Food Delivery Service' : 'Hotel Service'
            }
        };
        ...
        // If Chapa fails, delete the pending order so user can try again
        await Order.findByIdAndDelete(order._id);
        console.error('Chapa Response Error:', chapaRes.data);
        res.status(400).json({ message: chapaRes.data.message || 'Payment Gateway Failed' });

    } catch (err) {
        // FIXED ERROR HANDLING HERE - Removed optional chaining (?.)
        let errorData = err.message;
        if (err.response && err.response.data) {
            errorData = err.response.data;
        }
        console.error('createChapaOrder Exception:', errorData);
...
// ... keep existing functions (verifyPayment, etc.)
// ──────────────────────────────────────────────────────────────
// CHAPA VERIFY
// ──────────────────────────────────────────────────────────────
